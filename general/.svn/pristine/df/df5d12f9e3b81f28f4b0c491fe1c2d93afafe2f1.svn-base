<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	<h4 class="page-header">会议管理>>会议收集</h4>
		...
		<div id="searchBody" class="panel-body collapse in">
			<form class="form-inline" id="searchForm">
				<div class="form-group">
			    	...
			    	<input type="text" class="form-control" id="filter_LIKES_name" name="filter_LIKES_name" placeholder="名称">
			  	</div>
...
<script type="text/javascript">
	...
	$(function() {
		...
		var queryUrl = "${ctx}/collect/jsonDate?" + $("#searchForm").serialize();
	...
	function cx() {
		// 刷新表格  
		dataTable.bootstrapTable('refresh', {url : "${ctx}/collect/jsonDate?" + $("#searchForm").serialize()});
	}
	...
	function toCreate() {
		$.get('${ctx}/collect/toCreate', function(data) {
			$("#myModal").html(data);
		});
		$('#myModal').modal('show');
	}
	...
	function toUpdate(id, action) {
		$.get('${ctx}/collect/toUpdate/' + action + '/' + id, function(data) {
			$("#myModal").html(data);
		});
		$('#myModal').modal('show');
	}
	...
	$("#delButton").click(function() {
		$(this).button('loading');
		$.get('${ctx}/collect/delete/' + delId, function(data) {
			if (data == "success") {
				cx();
